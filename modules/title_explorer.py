    ...
    card_html = textwrap.dedent(f"""
        <div style="background-color: #1f1f1f; padding: 28px; border-radius: 12px; border: 1px solid #333; margin-bottom: 25px;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">
                <span style="background-color: {badge_color}; color: #ffffff; padding: 4px 14px; border-radius: 14px; font-size: 13px; font-weight: bold;">
                    {selected_item['type']}
                </span>
                <span style="color: #aaaaaa; font-size: 14px;">Date Added: {added_date_str}</span>
            </div>
            <h1 style="margin-top: 5px; margin-bottom: 8px; color: #ffffff !important; font-size: 32px;">
                {selected_item['title']}
            </h1>
            ...
            <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 12px; font-size: 14px;">
                <div><strong>🎭 Genres:</strong> <span style="color: #aaaaaa;">{selected_item['listed_in']}</span></div>
                <div><strong>🎬 Director:</strong> <span style="color: #aaaaaa;">{selected_item['director']}</span></div>
                <div><strong>🌟 Cast:</strong> <span style="color: #aaaaaa;">{selected_item['cast']}</span></div>
                <div><strong>🌎 Country:</strong> <span style="color: #aaaaaa;">{selected_item['country']}</span></div>
            </div>
        </div>
    """)
        ...
        for idx, (_, sim_row) in enumerate(similar_df.iterrows()):
            with rec_cols[idx]:
                rec_card_html = textwrap.dedent(f"""
                    <div style="background-color: #181818; padding: 18px; border-radius: 10px; border-top: 4px solid #E50914; min-height: 150px; height: 100%; box-sizing: border-box;">
                        <span style="font-size: 11px; color: #888888; text-transform: uppercase; font-weight: bold; letter-spacing: 0.5px;">
                            {sim_row['type']}
                        </span>
                        <h4 style="color: #ffffff !important; margin-top: 4px; margin-bottom: 6px; font-size: 16px;">
                            {sim_row['title']}
                        </h4>
                        <p style="color: #E50914; font-size: 12px; font-weight: bold; margin-bottom: 8px;">
                            {sim_row['release_year']} &nbsp;|&nbsp; {sim_row['duration']}
                        </p>
                        <p style="font-size: 12px; color: #aaaaaa; margin-bottom: 0; line-height: 1.4;">
                            <strong>Genres:</strong> {sim_row['listed_in']}
                        </p>
                    </div>
                """)
